...
// Algoritmo Möller–Trumbore. Retorna t (distancia) o -1 si no hay intersección.
double RayTracer::intersectRayTriangle(
    ...
    if (a > -EPSILON && a < EPSILON) return -1.0; // rayo paralelo al triángulo
...
Vec3 RayTracer::reflect(const Vec3& direction, const Vec3& normal) {
    ...
    return normalize({
        ...
        direction.z - d * normal.z
    });
}

// Distribución uniforme de direcciones en una esfera usando espiral de Fibonacci.
...
RayTracer::TraceOutput RayTracer::trace(
    ...
    // Aplanar triángulos con su absorción de superficie
    struct FlatTriangle {
        TriangleData tri;
        double       absorption;
    };

    std::vector<FlatTriangle> allTris;
    ...
    for (const Vec3& rayDir : rayDirs) {
        ...
        while (energy > ENERGY_MINIMUM && traveled < maxDist) {
            ...
            traveled       += nearestT; // direction es unitario, entonces t == distancia
            int timeMs      = static_cast<int>((traveled / config.soundSpeed) * 1000.0);

            // Comprobar receptores a lo largo de este segmento
            for (const auto& recv : receivers) {
                Vec3   op = subtract(recv.position, origin);
                double t  = dot(op, direction);

                if (t < 0.0 || t > nearestT) continue; // fuera del segmento

                Vec3   closest = add(origin, scale(direction, t));
                double dist    = length(subtract(recv.position, closest));
                ...
                if (dist <= recv.radius) {
                    ...
                    if (recvTimeMs <= config.durationMs) {
                        ReceiverEnergySample sample;
                        sample.receiverId = recv.id;
                        sample.timeMs     = recvTimeMs;
                        sample.energy     = energy;
                        output.receiverEnergy.push_back(sample);
                    }
                }
            }

            // Guardar segmento de rayo
            if (timeMs <= config.durationMs) {
                ReflectionRaySegment seg;
                seg.from   = origin;
                seg.to     = hitPoint;
                seg.energy = energy;
                seg.timeMs = timeMs;
                output.rays.push_back(seg);
            }

            // Aplicar absorción y separar energía difusa
            double remaining = energy * (1.0 - allTris[nearestIdx].absorption);
            double specular  = remaining * (1.0 - config.diffusionCoefficient);
